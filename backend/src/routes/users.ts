...
export default async function userRoutes(fastify: FastifyInstance, options: FastifyPluginOptions) {
  ...
  fastify.get('/', { preHandler: [fastify.authenticate, fastify.authorize(['admin', 'commander'])] }, async (request, reply) => {
    const query = `
      SELECT 
        u.id, u.username, u.full_name, u.rank, u.position, u.email, u.created_at,
  ...
  fastify.post('/', { preHandler: [fastify.authenticate, fastify.authorize(['admin', 'commander'])] }, async (request: any, reply) => {
    const { username, password, full_name, rank, position, email, role_id, unit_id } = request.body;
    ...
    try {
      ...
      const query = `
        INSERT INTO users (username, password_hash, full_name, rank, position, email, role_id, unit_id)
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
        RETURNING id, username, full_name, email
      `;
      const result = await fastify.pg.query(query, [username, password_hash, full_name || '', rank || '', position || '', email || '', role_id || 5, unit_id]);
  ...
  // 4. Update user
  fastify.patch('/:id', { preHandler: [fastify.authenticate, fastify.authorize(['admin', 'commander'])] }, async (request: any, reply) => {
    const { id } = request.params;
    const { full_name, rank, position, role_id, unit_id, email, username, password } = request.body as any;
    ...
    try {
      ...
      const query = `
        ...
        RETURNING *
      `;
      const result = await fastify.pg.query(query, [full_name, rank, position, role_id, unit_id, email, username, password_hash, id]);
  ...
  // 5. Delete user
  fastify.delete('/:id', { preHandler: [fastify.authenticate, fastify.authorize(['admin', 'commander'])] }, async (request: any, reply) => {
    const { id } = request.params;
  ...
  fastify.get('/registrations', { preHandler: [fastify.authenticate, fastify.authorize(['admin', 'commander'])] }, async (request, reply) => {
    const query = `
  ...
  fastify.post('/registrations/:id/approve', { preHandler: [fastify.authenticate, fastify.authorize(['admin', 'commander'])] }, async (request: any, reply) => {
    const { id } = request.params;
  ...
  // 9. Bulk delete users
  fastify.post('/bulk-delete', { preHandler: [fastify.authenticate, fastify.authorize(['admin', 'commander'])] }, async (request: any, reply) => {
