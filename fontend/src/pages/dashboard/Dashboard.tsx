...
import { DelegationManagement } from '../users/DelegationManagement'
import RoutineManagement from '../routines/RoutineManagement'
...
export default function Dashboard({ user, onLogout }: { user: any, onLogout: () => void }) {
  ...
  const [activeBroadcast, setActiveBroadcast] = useState<{ title: string, channel: string, user: string, channel_id?: number, schedule_id?: number, radio_id?: number, needsUnlock?: boolean, isPaused?: boolean, isHidden?: boolean, duration?: number, file_size?: string } | null>(null);
  ...
  const [pendingRoutine, setPendingRoutine] = useState<{ routineId: number, title: string } | null>(null);
  ...
  const normalizeBroadcastData = (data: any) => {
    ...
    return {
      ...
      needsUnlock: false,
      duration: data.duration || content.duration || 0,
      start_time: data.start_time || content.start_time || null,
      file_size: data.file_size || content.file_size || ''
    };
  };
  ...
  useEffect(() => {
    const handleOpenRoutine = (e: any) => {
      const { routineId, title } = e.detail;
      console.log('[Dashboard] Received openRoutineSchedule event:', { routineId, title });
      setPendingRoutine({ routineId, title });
      setActiveTab('schedule');
    };
    window.addEventListener('openRoutineSchedule', handleOpenRoutine);
    return () => window.removeEventListener('openRoutineSchedule', handleOpenRoutine);
  }, []);
  ...
  const fetchNotifications = async () => {
    try {
      const [notifsRes, countRes] = await Promise.all([
        ...
        fetch(`${API_URL}/notifications/unread-count`, { headers: getHeaders() })
      ]);
  ...
  const fetchEmergencyStatus = async () => {
    try {
      const res = await fetch(`${API_URL}/schedules/emergency/status`, { headers: getHeaders() });
      if (res.ok) {
        const data = await res.json();
  ...
  useEffect(() => {
    ...
    const connectWS = () => {
      ...
      statusWs.onopen = () => {
        ...
        // Identify as global monitor to receive binary stream from ALL channels
        statusWs!.send(JSON.stringify({
          type: 'identify',
          channel_id: 0, // 0 = Global Monitor
          device_id: 999,
          device_name: 'Admin Dashboard Monitor',
          isBrowser: true
        }));
      };
    ...
    <div className="dashboard-container ani-fade-in">
      ...
      <header className="dashboard-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: '18px' }}>
          ...
          <div>
            <span style={{ fontSize: '1.6rem', fontWeight: 900, letterSpacing: '-0.5px', display: 'block', lineHeight: 1, color: 'white', whiteSpace: 'nowrap' }}>TRUYỀN THANH NỘI BỘ</span>
            <span style={{ fontSize: '0.75rem', color: '#6366f1', textTransform: 'uppercase', letterSpacing: '2px', fontWeight: 800, marginTop: '4px', display: 'block' }}>HỆ THỐNG ĐIỀU HÀNH V2.1</span>
          </div>
        </div>
      ...
      <div className="dashboard-body">
        <aside className={`sidebar ${isSidebarCollapsed ? 'collapsed' : ''}`} style={{ position: 'relative', flexShrink: 0 }}>
          ...
          <nav style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
            {/* QUICK ACTIONS */}
            <div style={{ padding: '0 0.8rem', marginBottom: '1.5rem', marginTop: '0.5rem', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <button 
                onClick={() => {
                  setActiveTab('content-mgmt');
                  setTimeout(() => window.dispatchEvent(new Event('openCreateBulletinModal')), 100);
                }} 
                className="btn-primary hover-scale" 
                style={{ display: 'flex', alignItems: 'center', justifyContent: isSidebarCollapsed ? 'center' : 'flex-start', gap: '8px', padding: '10px', background: 'linear-gradient(135deg, #10b981, #059669)', border: 'none', boxShadow: '0 4px 15px -3px rgba(16, 185, 129, 0.4)', borderRadius: '12px' }}
                title={isSidebarCollapsed ? "Tạo Bản tin mới" : ""}
              >
                <Plus size={isSidebarCollapsed ? 20 : 18} />
                {!isSidebarCollapsed && <span style={{ fontWeight: 700, fontSize: '0.9rem' }}>Tạo Bản tin mới</span>}
              </button>
            </div>
            ...
            <AnimatePresence initial={false}>
              {(expandedSections.chung || isSidebarCollapsed) && (
                ...
                >
                  ...
                  <div className={`nav-item ${activeTab === 'analytics' ? 'active' : ''}`} onClick={() => setActiveTab('analytics')} title={isSidebarCollapsed ? "Thống kê tin bài" : ""}>
                    <TrophyIcon size={20} />
                    {!isSidebarCollapsed && <span>Thống kê tin bài</span>}
                  </div>
                  {(user?.role_name?.toLowerCase() === 'admin' || user?.role_name?.toLowerCase() === 'quản trị viên' || user?.id === 1) && (
            ...
            <AnimatePresence initial={false}>
              {(expandedSections.vanhanh || isSidebarCollapsed) && (
                <motion.div
                  ...
                  style={{ overflow: 'hidden' }}
                >
                  {(user?.role_name?.toLowerCase() === 'admin' || user?.role_name?.toLowerCase() === 'quản trị viên' || user?.id === 1 || user?.role_name?.toLowerCase() === 'technician' || user?.role_name?.toLowerCase() === 'commander' || user?.role_name?.toLowerCase() === 'quản lý') && (
                    <div className={`nav-item ${activeTab === 'schedule' ? 'active' : ''}`} onClick={() => setActiveTab('schedule')} title={isSidebarCollapsed ? "Lịch phát thanh" : ""}>
                      <Calendar size={20} />
                      {!isSidebarCollapsed && <span>Lịch phát thanh</span>}
                    </div>
                  )}
                  {(user?.role_name?.toLowerCase() === 'admin' || user?.role_name?.toLowerCase() === 'quản trị viên' || user?.id === 1 || user?.role_name?.toLowerCase() === 'technician' || user?.role_name?.toLowerCase() === 'commander' || user?.role_name?.toLowerCase() === 'quản lý') && (
                    <>
                      <div className={`nav-item ${activeTab === 'radio' ? 'active' : ''}`} onClick={() => setActiveTab('radio')} title={isSidebarCollapsed ? "Tiếp sóng Radio" : ""}>
                        <Radio size={20} />
                        {!isSidebarCollapsed && <span>Tiếp sóng Radio</span>}
                      </div>
                      <div className={`nav-item ${activeTab === 'devices' ? 'active' : ''}`} onClick={() => setActiveTab('devices')} title={isSidebarCollapsed ? "Quản lý thiết bị" : ""}>
                        <Speaker size={20} />
                        {!isSidebarCollapsed && <span>Quản lý thiết bị</span>}
                      </div>
                    </>
                  )}
                  {(user?.role_name?.toLowerCase() === 'admin' || user?.role_name?.toLowerCase() === 'quản trị viên' || user?.id === 1 || user?.role_name?.toLowerCase() === 'technician' || user?.role_name?.toLowerCase() === 'commander' || user?.role_name?.toLowerCase() === 'quản lý' || user?.role_name === 'broadcaster') && (
                    <div className={`nav-item ${activeTab === 'routines' ? 'active' : ''}`} onClick={() => setActiveTab('routines')} title={isSidebarCollapsed ? "Hiệu lệnh làm việc" : ""}>
                      <Clock size={20} />
                      {!isSidebarCollapsed && <span>Hiệu lệnh làm việc</span>}
                    </div>
                  )}
                </motion.div>
              )}
            ...
            <AnimatePresence initial={false}>
              {(expandedSections.bientap || isSidebarCollapsed) && (
                <motion.div
                  ...
                  style={{ overflow: 'hidden' }}
                >
                  {(user?.role_name?.toLowerCase() === 'admin' || user?.role_name?.toLowerCase() === 'quản trị viên' || user?.id === 1 || user?.role_name?.toLowerCase() === 'editor' || user?.role_name?.toLowerCase() === 'commander' || user?.role_name?.toLowerCase() === 'quản lý') && (
                   ...
                   {(user?.role_name?.toLowerCase() === 'admin' || user?.role_name?.toLowerCase() === 'quản trị viên' || user?.id === 1 || user?.role_name === 'technician' || user?.role_name === 'commander' || user?.role_name?.toLowerCase() === 'quản lý' || user?.role_name === 'broadcaster') && (
                  ...
                  {(user?.role_name?.toLowerCase() === 'admin' || user?.role_name?.toLowerCase() === 'quản trị viên' || user?.id === 1 || user?.role_name === 'editor') && (
            ...
            <AnimatePresence initial={false}>
              {(expandedSections.quantri || isSidebarCollapsed) && (
                <motion.div
                  ...
                  style={{ overflow: 'hidden' }}
                >
                  {(user?.role_name?.toLowerCase() === 'admin' || user?.role_name?.toLowerCase() === 'quản trị viên' || user?.id === 1 || user?.role_name === 'commander' || user?.role_name?.toLowerCase() === 'quản lý') && (
                  ...
                  {(user?.role_name?.toLowerCase() === 'admin' || user?.role_name?.toLowerCase() === 'quản trị viên' || user?.id === 1) && (
                    <div className={`nav-item ${activeTab === 'delegation' ? 'active' : ''}`} onClick={() => setActiveTab('delegation')} title={isSidebarCollapsed ? "Ủy quyền (Delegation)" : ""}>
                      <Shield size={20} />
                      {!isSidebarCollapsed && <span>Ủy Quyền</span>}
                    </div>
                  )}
                  {(user?.role_name?.toLowerCase() === 'admin' || user?.role_name?.toLowerCase() === 'quản trị viên' || user?.id === 1) && (
        ...
        <main className="main-content" style={{ position: 'relative', flexGrow: 1, minWidth: 0 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '2px' }}>
                        ...
                        {!isAudioEnabled && (
                          <button 
                            ...
                            style={{ background: '#f59e0b', color: 'white', border: 'none', padding: '2px 8px', borderRadius: '4px', fontSize: '0.7rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}
                          >
                            <Volume2 size={12} /> NGHE BẢN TIN
                          </button>
                        )}
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#64748b', fontSize: '0.8rem' }}>
                        ...
                        {activeBroadcast.file_size && (
                          <>
                            <span style={{ opacity: 0.3 }}>|</span>
                            <span>Dung lượng: <strong style={{ color: '#818cf8' }}>{activeBroadcast.file_size}</strong></span>
                          </>
                        )}
                      </div>
                    </div>
              ...
              {(() => {
                try {
                  ...
                  switch (activeTab) {
                        ...
                        <div className="animate-fade-in">
                          ...
                          <section className="section-container">
                            <div className="stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1.5rem' }}>
                              ...
                              <div className="stat-card" style={{ cursor: 'pointer', borderTop: '4px solid #6366f1' }} onClick={() => setActiveTab('schedule')}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    case 'schedule': return <ScheduleManagement user={user} onLogout={onLogout} activeBroadcast={activeBroadcast} onStopBroadcast={stopBroadcast} onStartBroadcast={handleStartBroadcast} pendingRoutine={pendingRoutine} onRoutineHandled={() => setPendingRoutine(null)} />;
                    case 'content': return <ContentManagement user={user} onLogout={onLogout} />;
                    case 'media': return <MediaLibrary onLogout={onLogout} />;
                    case 'radio': return <RadioManagement onLogout={onLogout} />;
                    case 'content-mgmt':
                      return (
                        <div className="animate-fade-in">
                    ...
                    case 'devices': return <DeviceManagement user={user} onLogout={onLogout} />;
                    case 'users': return <UserManagement user={user} onLogout={onLogout} />;
                    case 'delegation': return <DelegationManagement user={user} />;
