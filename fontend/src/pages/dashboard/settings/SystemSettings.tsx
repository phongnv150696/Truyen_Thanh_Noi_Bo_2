...
export default function SystemSettings() {
  ...
  const handleUpdateConfig = async (key: string, value: string) => {
    setSaving(key);
    try {
      const res = await fetch(`http://localhost:3000/settings/${key}`, {
        method: 'PATCH',
                    ...
                    <tbody>
                      {healthMetrics.map(metric => (
                        <tr key={metric.id} className="table-row-hover" style={{ fontSize: '0.9rem', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                          ...
                          <td style={{ padding: '1.2rem 1.5rem', textAlign: 'right', color: '#64748b' }}>{new Date(metric.recorded_at).toLocaleTimeString()}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
