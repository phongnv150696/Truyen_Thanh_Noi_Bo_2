...
class AudioRateController:
            ...
            if item_type == "audio":
                ...
                if elapsed_ms < output_ms:
                    ...
                    try:
                        ...
                        self.logger.bind(tag=TAG).debug("Task gửi audio bị hủy")
                        raise
                ...
                try:
                    ...
                    self.logger.bind(tag=TAG).error(f"Gửi audio thất bại: {e}")
                    raise
    ...
    async def start_sending(self, send_audio_callback, send_message_callback=None):
        ...
        async def _send_loop():
            try:
                ...
                self.logger.bind(tag=TAG).info("Chu kỳ gửi audio đã dừng")
            except Exception as e:
                self.logger.bind(tag=TAG).error(f"Lỗi chu kỳ gửi audio: {e}")
    ...
    def stop_sending(self):
        ...
        if self.pending_send_task and not self.pending_send_task.done():
            self.pending_send_task.cancel()
            self.logger.bind(tag=TAG).debug("Đã hủy task gửi audio")
