                ...
                if response.status_code == 200:
                    ...
                    if isinstance(result, dict):
                        if result.get("code") == 0:
                            ...
                            self.logger.bind(tag=TAG).warning(f"API {url} trả về lỗi: {result.get('msg')}")
                    else:
                        self.logger.bind(tag=TAG).warning(f"API {url} không trả về JSON dict")
                else:
                    self.logger.bind(tag=TAG).warning(f"Yêu cầu API {url} thất bại: {response.status_code}")
            except Exception as e:
                self.logger.bind(tag=TAG).error(f"Lấy dữ liệu context {url} thất bại: {e}")
        
        # 将所有格式化后的行拼接成一个字符串
        self.context_data = "\n".join(formatted_lines)
        if self.context_data:
            self.logger.bind(tag=TAG).debug(f"Đã inject dữ liệu context:\n{self.context_data}")
