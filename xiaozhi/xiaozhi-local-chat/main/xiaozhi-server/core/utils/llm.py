...
def create_instance(class_name, *args, **kwargs):
    # 创建LLM实例
    # Special handling for RAG provider
    if class_name == 'rag':
        lib_name = 'core.providers.llm.rag.rag_ollama'
        if lib_name not in sys.modules:
            sys.modules[lib_name] = importlib.import_module(lib_name)
        logger.bind(tag=__name__).info(f"Loading RAG LLM Provider")
        return sys.modules[lib_name].LLMProvider(*args, **kwargs)
    
    # Standard provider loading
