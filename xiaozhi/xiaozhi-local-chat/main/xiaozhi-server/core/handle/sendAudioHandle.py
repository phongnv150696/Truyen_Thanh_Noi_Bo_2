...
async def sendAudioMessage(conn, sentenceType, audios, text):
    # MUTEX: Block TTS audio if audio template is playing
    if hasattr(conn, 'audio_template_playing') and conn.audio_template_playing:
        conn.logger.bind(tag=TAG).warning(f"Skipping TTS audio, audio template is playing: {text}")
        return  # Discard this TTS audio
    
    if conn.tts.tts_audio_first_sentence:
        conn.logger.bind(tag=TAG).info(f"Gửi đoạn audio đầu tiên: {text}")
    ...
    if sentenceType is not SentenceType.MIDDLE:
        conn.logger.bind(tag=TAG).info(f"Gửi tin nhắn audio: {sentenceType}, {text}")
...
async def _do_send_audio(conn, opus_packet, flow_control, frame_duration=60):
    ...
    if conn.conn_from_mqtt_gateway:
        ...
        # Check protocol version for V3 header
        protocol_version = getattr(conn, "protocol_version", 0)
        if protocol_version == 3:
            # V3协议: 4字节头部 (Type=0, Reserved=0, PayloadLen=2bytes big-endian)
            header = bytearray(4)
            header[0] = 0  # Type: Audio
            header[1] = 0  # Reserved
            header[2:4] = len(opus_packet).to_bytes(2, "big")  # PayloadLen
